...
for (cur_oktmo in oktmo_set$oktmo){
  ...
  for (col_name in predict_cols){
    ...
    if (sd(df_lm_part[[col_name]]) == 0 | sd(col_prev_data) == 0){
      shift_prev <- 1
    }
    else{
    ...
    if (nrow(df_lm_part) > 0 & sum(df_lm_part[[col_name]]) != 0)
    {
      ...
      if (dim(sf$coefficients)[1] < 9 | last(sf$coefficients[, 1]) < 0)
      {
        lm_formula <- paste(col_name,
                            " ~ date_int + week")
...
w_except_oktmo <- except_oktmo2


w_exc_cnt <- exc_cnt
w_except_col <- except_col
w_except_oktmo <- except_oktmo

for (ind in 1:w_exc_cnt){
  col_name <- w_except_col[ind]
  cur_oktmo <- w_except_oktmo[ind]
